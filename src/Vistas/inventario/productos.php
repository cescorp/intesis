                ...
                <section class="panel-crud">
                    ...
                    <div class="table-responsive">
                        <table id="tablaProductos" class="table table-hover tabla-intesis align-middle w-100">
                            <thead><tr><th>Codigo</th><th>Nombre</th><th>Categoria</th><th>Marca</th><th>Unidad</th><th>IVA</th><th>Costo ultimo</th><th>Stock minimo</th><th>Estado</th><th class="text-end">Acciones</th></tr></thead>
                            <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end acciones-tabla">
                                            ...
                                            <?php if ($producto['sis_estado_codigo'] === 'ACTIVO' && $permisos['inactivar']): ?>
                                                <form action="<?= $appUrl ?>/inventario/productos/inactivar" method="post" class="d-inline formulario-confirmar"><input type="hidden" name="producto_id" value="<?= (int) $producto['inv_producto_id'] ?>"><button type="submit" class="btn btn-accion btn-inactivar" title="Inactivar producto"><i class="bi bi-toggle-off"></i></button></form>
                                            <?php endif; ?>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
        ...
        <form class="modal-content" id="formularioProducto" method="post" action="<?= $appUrl ?>/inventario/productos/crear">
            ...
            <div class="modal-body formulario-compacto">
                <div class="row g-2">
                    ...
                    <?php endif; ?>
                    <div class="col-md-3"><label class="form-label" for="producto_codigo_principal">Codigo</label><input type="text" class="form-control form-control-sm" id="producto_codigo_principal" name="codigo_principal" maxlength="40" required></div>
                    <div class="col-md-3"><label class="form-label" for="producto_codigo_auxiliar">Codigo auxiliar</label><input type="text" class="form-control form-control-sm" id="producto_codigo_auxiliar" name="codigo_auxiliar" maxlength="40"></div>
                    <div class="<?= $esSuperusuario ? 'col-md-6' : 'col-md-6' ?>"><label class="form-label" for="producto_nombre">Nombre</label><input type="text" class="form-control form-control-sm" id="producto_nombre" name="nombre" maxlength="180" required></div>
                    <div class="col-md-4"><label class="form-label" for="producto_categoria_id">Categoria</label><div class="input-group input-group-sm"><select class="form-control producto-select-empresa" id="producto_categoria_id" name="categoria_id" required><option value="">Seleccione</option><?php foreach ($categorias as $categoria): ?><option value="<?= (int) $categoria['inv_categoria_id'] ?>" data-empresa="<?= (int) $categoria['sis_empresa_id'] ?>"><?= htmlspecialchars($categoria['inv_categoria_nombre']) ?></option><?php endforeach; ?></select><?php if ($permisos['crear_categoria']): ?><button class="btn btn-outline-secondary" type="button" data-bs-toggle="modal" data-bs-target="#modalCategoriaRapida" title="Nueva categoria"><i class="bi bi-plus-lg"></i></button><?php endif; ?></div></div>
                    <div class="col-md-4"><label class="form-label" for="producto_marca_id">Marca</label><div class="input-group input-group-sm"><select class="form-control producto-select-empresa" id="producto_marca_id" name="marca_id" required><option value="">Seleccione</option><?php foreach ($marcas as $marca): ?><option value="<?= (int) $marca['inv_marca_id'] ?>" data-empresa="<?= (int) $marca['sis_empresa_id'] ?>"><?= htmlspecialchars($marca['inv_marca_nombre']) ?></option><?php endforeach; ?></select><?php if ($permisos['crear_marca']): ?><button class="btn btn-outline-secondary" type="button" data-bs-toggle="modal" data-bs-target="#modalMarcaRapida" title="Nueva marca"><i class="bi bi-plus-lg"></i></button><?php endif; ?></div></div>
                    <div class="col-md-2"><label class="form-label" for="producto_costo_ultimo">Costo ultimo</label><input type="number" min="0" step="0.0001" class="form-control form-control-sm" id="producto_costo_ultimo" name="costo_ultimo" value="0"></div>
                    <div class="col-md-2"><label class="form-label" for="producto_stock_minimo">Stock minimo</label><input type="number" min="0" step="0.0001" class="form-control form-control-sm" id="producto_stock_minimo" name="stock_minimo" value="0"></div>
                    <div class="col-md-2"><label class="form-label" for="producto_stock_maximo">Stock maximo</label><input type="number" min="0" step="0.0001" class="form-control form-control-sm" id="producto_stock_maximo" name="stock_maximo" value="0"></div>
                    <div class="col-md-2"><div class="form-check mt-4"><input class="form-check-input" type="checkbox" id="producto_lleva_iva" name="lleva_iva" value="1" checked><label class="form-check-label" for="producto_lleva_iva">IVA</label></div></div>
                    <div class="col-12"><label class="form-label" for="producto_descripcion">Descripcion</label><textarea class="form-control form-control-sm" id="producto_descripcion" name="descripcion" rows="2"></textarea></div>
                </div>
            </div>
