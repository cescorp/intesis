        ...
        <form class="modal-content" id="formularioProducto" method="post" action="<?= $appUrl ?>/inventario/productos/crear">
            ...
            <div class="modal-body formulario-compacto">
                <div class="row g-2">
                    ...
                    <div class="col-md-3" id="filaProveedorProducto"><label class="form-label" for="producto_proveedor_id">Proveedor (opcional)</label><select class="form-control form-control-sm producto-select-empresa" id="producto_proveedor_id" name="proveedor_id"><option value="">Seleccione</option><?php foreach ($proveedores as $proveedor): ?><option value="<?= (int) $proveedor['com_proveedor_id'] ?>" data-empresa="<?= (int) $proveedor['sis_empresa_id'] ?>"><?= htmlspecialchars($proveedor['com_proveedor_razon_social']) ?></option><?php endforeach; ?></select></div>
                    <div class="col-md-3" id="filaCodProveedorProducto"><label class="form-label" for="producto_cod_proveedor">Cod. Proveedor (opcional)</label><input type="text" class="form-control form-control-sm" id="producto_cod_proveedor" name="cod_proveedor" maxlength="40"></div>
...
window.INTESIS_SIGUIENTE_CODIGO_PRODUCTO = <?= json_encode($siguienteCodigoSugerido ?? '1') ?>;
</script>
