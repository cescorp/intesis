...
SELECT 'PDF Proforma', m.sis_menu_id, 'bi bi-dot', '/ventas/proformas/pdf', 7, 1, 'B', 1
FROM sis_menu m
WHERE m.sis_menu_url = '/ventas/proformas'
  AND NOT EXISTS (SELECT 1 FROM sis_menu WHERE sis_menu_url = '/ventas/proformas/pdf');
...
WHERE lower(p.sis_perfil_nombre) = 'superusuario'
  AND sm.sis_menu_url = '/ventas/proformas/pdf'
  AND NOT EXISTS (
